...
public class FlyingGenieLevelGenieTransform : LevelProperties.FlyingGenie.Entity
{
	...
	private IEnumerator bombs_cr()
	{
		...
		while (true)
		{
			for (stringCounter2 = 0; stringCounter2 < bombPattern.Length; stringCounter2++)
			{
				state = State.Marionette;
				...
				for (int i = 0; i < bombType.Length; i++)
				{
					...
					else if (bombType[i][0] == 'D')
					{
						picked = diagonalBomb;
					}
					else if (bombType[i][0] == 'P')
					{
						picked = plusSizedBomb;
					}
					string[] bombPos = bombType[i].Split('-');
					for (int j = 0; j < bombPos.Length; j++)
					{
						float.TryParse(bombPos[j], out location);
						if (j % 2 == 0)
						{
							endPosition.x = -640f + location;
						}
	...
	private IEnumerator shoot_cr()
	{
		...
		while (state == State.Marionette)
		{
			...
			{
				for (int j = 0; j < int.Parse(shotCount[shotIndex]); j++)
				{
