...
public class FlowerLevelFlower : LevelProperties.Flower.Entity
{
	...
	private IEnumerator spawnGattlingGunSeeds_cr()
	{
		...
		for (int i = 0; i < currentGattlingGunAttackPattern.Count; i++)
		{
			...
			if (t == 'D')
			{
				yield return CupheadTime.WaitForSeconds(this, float.Parse(currentGattlingGunAttackPattern[i].Substring(1)));
				continue;
			}
			if (miniFlowerSpawned)
			{
				if (t != 'C')
				{
					SpawnEnemySeed(int.Parse(currentGattlingGunAttackPattern[i].Substring(1)), t);
				}
				else
				{
					SpawnEnemySeed(int.Parse(currentGattlingGunAttackPattern[i].Substring(1)), t, false);
				}
			}
			else
			{
	...
	private IEnumerator spawnBullets_cr()
	{
		...
		for (int j = 0; j < base.properties.CurrentState.bullets.numberOfProjectiles; j++)
		{
			...
			bullets[j].GetComponent<FlowerLevelSeedBullet>().OnBulletSeedStart(this, PlayerManager.GetNext(), base.properties.CurrentState.bullets.acceleration, base.properties.CurrentState.bullets.speedMinMax.min, base.properties.CurrentState.bullets.speedMinMax.max, base.properties.CurrentState.bullets.delayNextShot);
