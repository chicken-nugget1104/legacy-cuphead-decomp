...
public class DicePalaceDominoLevelDomino : LevelProperties.DicePalaceDomino.Entity
{
	...
	private IEnumerator spawn_ball_cr()
	{
		...
		proj.GetComponent<DicePalaceDominoLevelBouncyBall>().InitBouncyBall(base.properties.CurrentState.bouncyBall.bulletSpeed, base.properties, floor, direction2);
	...
	private IEnumerator spawn_boomerang_cr()
	{
		...
		{
			DicePalaceDominoLevelBoomerang proj2 = boomerangPrefab.Create(birdSpawnpoint.position, p.boomerangSpeed, p.health, p);
		}
		else
		{
			DicePalaceDominoLevelBoomerang proj2 = boomerangPrefab.Create(birdSpawnpoint.position, p.boomerangSpeed, p.health, p);
			proj2.GetComponent<SpriteRenderer>().color = Color.magenta;
			proj2.SetParryable(true);
		}
