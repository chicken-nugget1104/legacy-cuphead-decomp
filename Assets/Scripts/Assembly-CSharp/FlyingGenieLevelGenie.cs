...
public class FlyingGenieLevelGenie : LevelProperties.FlyingGenie.Entity
{
	...
	private IEnumerator small_gems_cr()
	{
		...
		while (!smallGemTimerUp && attackLooping)
		{
			...
			float.TryParse(smallOffsetString2[offsetIndex], out offset);
	...
	private IEnumerator sphinx_cr()
	{
		...
		string[] sphinxPosXPattern2 = p.sphinxAimX[mainXIndex].Split(',');
		int posXIndex = Random.Range(0, sphinxPosXPattern2.Length);
		...
		while (attackLooping)
		{
			...
			sphinxPosXPattern2 = p.sphinxAimX[mainXIndex].Split(',');
			...
			{
				float.TryParse(sphinxPosXPattern2[posXIndex], out posX);
				float.TryParse(sphinxPosXPattern2[posYIndex], out posY);
				endPos.x = -640f + posX;
				endPos.y = 360f - posY;
				SpawnSphinx(endPos);
	...
	private IEnumerator coffin_cr()
	{
		...
		string[] coffinPosPattern2 = p.mummyAppearString[mainPosIndex].Split(',');
		int posIndex = Random.Range(0, coffinPosPattern2.Length);
		...
		while (attackLooping)
		{
			coffinPosPattern2 = p.mummyAppearString[mainPosIndex].Split(',');
			coffinTypePattern2 = p.mummyTypeString[mainTypeIndex].Split(',');
			coffinAnglePattern2 = p.mummyGenieDirection[mainAngleIndex].Split(',');
			float.TryParse(coffinPosPattern2[posIndex], out position);
			float.TryParse(coffinAnglePattern2[angleIndex], out angle);
			...
			if (posIndex < coffinPosPattern2.Length - 1)
			{
				posIndex++;
			}
			else
